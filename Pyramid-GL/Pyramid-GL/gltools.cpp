...
GLbyte *gltLoadTGA(const char *szFileName, GLint *iWidth, GLint *iHeight, GLint *iComponents, GLenum *eFormat)
    ...
    *iComponents = GL_RGB8;
    ...
    if(pFile == NULL){
        std::cout <<" Error: File name " << szFileName << " is not found.\n";
        return NULL;
    }
    ...
    *iHeight = tgaHeader.height;
    ...
    if(tgaHeader.bits != 8 && tgaHeader.bits != 24 && tgaHeader.bits != 32) {
        std::cout << "Error: " << "tga file is not satified!\n";
        return NULL;
        }
    ...
    pBits = (GLbyte*)malloc(lImageSize * sizeof(GLbyte));
            if(pBits == NULL) {
                std::cout << "Error: " << "malloc memory failed!\n";
                return NULL;
            }
    ...
    if(fread(pBits, lImageSize, 1, pFile) != 1)
		{
            std::cout << "Error: "<< " read(pBits, lImageSize, 1, pFile) != 1" << "\n";
        free(pBits);
        return NULL;
		}
